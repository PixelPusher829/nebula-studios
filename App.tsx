...
type Page = '/' | '/games' | '/studio' | '/news' | '/careers' | '/press' | '/contact';
...
export const NavigationContext = createContext<NavContextType>({ currentPath: '/', navigate: () => {} });
      ...
      {isOpen && (
        <div className="bg-white rounded-lg shadow-2xl w-80 overflow-hidden flex flex-col h-96 border border-gray-200">
           <div className="bg-studio-black p-4 flex justify-between items-center text-white">
             <span className="font-bold text-sm uppercase">Studio Assistant</span>
             <button onClick={() => setIsOpen(false)}><X className="w-4 h-4" /></button>
           </div>
           
           <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
             {messages.length === 0 && <p className="text-xs text-gray-500 text-center mt-4">Ask about our games, careers, or support.</p>}
             {messages.map(m => (
               <div key={m.id} className={`p-2 rounded text-sm max-w-[85%] ${m.role === 'user' ? 'bg-studio-accent text-white ml-auto' : 'bg-gray-200 text-gray-800'}`}>
                 {m.text}
               </div>
             ))}
...
function App() {
  ...
  const renderPage = () => {
    switch (currentPath) {
      case '/': return <Hero />;
      case '/games': return <GamePortfolio />;
      case '/studio': return <StudioLife />;
