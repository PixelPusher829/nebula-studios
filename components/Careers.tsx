...
import { ArrowRight, MapPin, Clock, Briefcase, X, Upload, Check } from 'lucide-react';
    ...
    <section className="pt-32 pb-24 bg-studio-dark relative min-h-screen">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-6">
          <div className="max-w-2xl">
            <h4 className="text-studio-accent font-display font-bold tracking-widest uppercase mb-2">Join The Nebula</h4>
            <h2 className="text-4xl md:text-5xl font-display font-bold text-white mb-6">BUILD THE IMPOSSIBLE</h2>
            <p className="text-studio-light font-sans text-lg leading-relaxed">
              We're looking for visionaries, rebels, and masters of their craft. If you want to create games that define a generation, you belong here.
            </p>
          </div>
        </div>
      ...
      {selectedJob && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
           <div className="absolute inset-0 bg-studio-black/90 backdrop-blur-md" onClick={closeJob} />
           <div className="relative w-full max-w-4xl bg-studio-card rounded-2xl overflow-hidden shadow-2xl border border-white/10 flex flex-col max-h-[90vh]">
              ...
              <div className="p-8 border-b border-white/10 flex justify-between items-start bg-studio-dark">
                <div>
                   <span className="text-studio-accent font-bold uppercase tracking-widest text-xs mb-2 block">{selectedJob.department}</span>
                   <h2 className="text-3xl font-display font-bold text-white mb-2">{selectedJob.title}</h2>
                ...
                <button onClick={closeJob} className="p-2 hover:bg-white/10 rounded-full text-white transition-colors">
                  <X className="w-6 h-6" />
                </button>
              </div>
                 ...
                 <div className="bg-studio-black/50 p-6 rounded-xl h-fit border border-white/5">
                    {!applicationSent ? (
                      <form onSubmit={handleApply} className="space-y-4">
                        <h3 className="text-white font-bold uppercase mb-4">Apply for this Role</h3>
                        <div>
                          <label className="text-xs text-studio-light uppercase font-bold block mb-1">Full Name</label>
                          <input required type="text" className="w-full bg-studio-card border border-white/10 rounded p-2 text-white focus:border-studio-accent outline-none" placeholder="Jane Doe" />
                        </div>
                        <div>
                          <label className="text-xs text-studio-light uppercase font-bold block mb-1">Email</label>
                          <input required type="email" className="w-full bg-studio-card border border-white/10 rounded p-2 text-white focus:border-studio-accent outline-none" placeholder="jane@example.com" />
                        </div>
                        <div>
                          <label className="text-xs text-studio-light uppercase font-bold block mb-1">Portfolio / LinkedIn</label>
                          <input type="url" className="w-full bg-studio-card border border-white/10 rounded p-2 text-white focus:border-studio-accent outline-none" placeholder="https://..." />
                        </div>
                        <div>
                           <label className="text-xs text-studio-light uppercase font-bold block mb-1">Resume / CV</label>
                           <div className="border border-dashed border-white/20 rounded p-4 text-center cursor-pointer hover:border-studio-accent hover:bg-studio-accent/5 transition-colors">
                              ...
                              <span className="text-xs text-studio-light">Click to upload (PDF)</span>
                           </div>
                        </div>
                        <button type="submit" className="w-full bg-studio-accent text-white py-3 rounded font-bold uppercase hover:bg-orange-600 transition-colors">
                          Submit Application
                        </button>
                      </form>
                    ) : (
                      <div className="text-center py-12">
                         ...
                         <p className="text-studio-light text-sm">Thanks for applying. We will be in touch shortly.</p>
                         <button onClick={closeJob} className="mt-6 text-studio-accent text-sm font-bold uppercase hover:text-white">Close</button>
                      </div>
                    )}
                 </div>
              </div>
