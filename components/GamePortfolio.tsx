...
import { Star, Monitor, Gamepad, Smartphone, X, Play, ShoppingCart } from 'lucide-react';
...
const games: Game[] = [
  {
    ...
    platforms: ['PC', 'PS5'],
    year: '2024',
    description: 'Dive into a shattered world where time is fractured. Master the Chrono-Blade and rewrite history in this award-winning Action RPG.',
  ...
  },
  {
    ...
    year: '2023',
    description: 'The ultimate 5v5 tactical shooter set in a neon-drenched metropolis. Choose your operative, master your abilities, and dominate the leaderboard.',
    features: ['120Hz Server Tick Rate', 'Cross-Platform Play', 'Seasonal Battle Pass']
  },
  {
    id: '3',
    ...
    platforms: ['PC', 'Mobile'],
    year: '2022',
    description: 'Defy gravity in the galaxy’s most dangerous racing league. Customize your ship and drift through asteroid fields at mach speeds.',
  ...
  },
  {
    ...
    platforms: ['PC', 'PS5'],
    year: '2023',
    description: 'You are alone on Station Theta. Something is hunting you. Resource management meets psychological horror in this terrifying experience.',
    ...
    <section className="pt-32 pb-24 bg-studio-dark relative min-h-screen">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex justify-between items-end mb-16">
          <div>
            <h4 className="text-studio-accent font-display font-bold tracking-widest uppercase mb-2">Our Portfolio</h4>
            <h2 className="text-4xl md:text-5xl font-display font-bold text-white">LATEST RELEASES</h2>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {games.map((game) => (
            <div key={game.id} className="group relative bg-studio-card rounded-xl overflow-hidden shadow-lg transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-studio-accent/10">
              {/* Image Container */}
              ...
              <div className="p-6 relative">
                ...
                <h3 className="text-xl font-display font-bold text-white mb-4 line-clamp-1 group-hover:text-studio-accent transition-colors">{game.title}</h3>
                
                <div className="pt-4 border-t border-white/5 flex justify-between items-center">
                  <div className="flex gap-3 text-studio-light">
                    {game.platforms.includes('PC') && <Monitor className="w-4 h-4" />}
                    {(game.platforms.includes('PS5') || game.platforms.includes('XSX')) && <Gamepad className="w-4 h-4" />}
                    {game.platforms.includes('Mobile') && <Smartphone className="w-4 h-4" />}
                  </div>
                  <span className="text-sm text-studio-light font-sans">{game.year}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      {selectedGame && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-studio-black/90 backdrop-blur-md" onClick={() => setSelectedGame(null)} />
          <div className="relative w-full max-w-5xl bg-studio-card rounded-2xl overflow-hidden shadow-2xl border border-white/10 flex flex-col md:flex-row max-h-[90vh]">
             ...
             <div className="md:w-1/2 p-8 md:p-12 overflow-y-auto bg-studio-card">
               <div className="flex justify-end mb-6 hidden md:flex">
                 <button onClick={() => setSelectedGame(null)} className="text-studio-light hover:text-white transition-colors">
                   <X className="w-6 h-6" />
                 </button>
               </div>
               ...
               <div className="space-y-6">
                 <div>
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <span className="text-studio-light text-sm font-sans">{selectedGame.year}</span>
                    </div>
                    <h2 className="text-4xl md:text-5xl font-display font-bold text-white mb-2 leading-none">{selectedGame.title}</h2>
                    <div className="flex items-center gap-2 text-yellow-500">
                       {[...Array(5)].map((_, i) => (
                         <Star key={i} className={`w-4 h-4 ${i < Math.floor(selectedGame.rating) ? 'fill-current' : 'text-gray-600'}`} />
                       ))}
                       <span className="text-white ml-2 text-sm font-bold">{selectedGame.rating}/5.0</span>
                    </div>
                 </div>
                 ...
                 <div className="space-y-3">
                   <h4 className="text-white font-bold uppercase tracking-widest text-sm">Key Features</h4>
                   <ul className="space-y-2">
                     {selectedGame.features?.map((feature, idx) => (
                       <li key={idx} className="flex items-center gap-2 text-studio-light text-sm">
