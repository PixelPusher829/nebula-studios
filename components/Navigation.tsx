...
export const Navigation: React.FC = () => {
  ...
  const navBackground = isHome && !scrolled ? 'bg-transparent border-transparent py-6' : 'bg-studio-black/95 border-b border-white/10 py-4 backdrop-blur-md';
    ...
    <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${navBackground}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          {/* Logo */}
          <a onClick={() => handleNavClick('/')} className="flex-shrink-0 flex items-center gap-3 cursor-pointer">
          ...
          <div className="hidden md:flex items-center space-x-8 lg:space-x-12">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  {item.label}
                  <span className={`absolute -bottom-2 left-0 h-0.5 bg-studio-accent transition-all duration-300 ${isActive ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
                </a>
              );
            })}
          </div>
          ...
          <div className="hidden md:flex items-center gap-6">
             ...
             <button onClick={() => handleNavClick('/careers')} className="bg-white text-studio-black px-6 py-2 rounded font-bold font-display text-sm uppercase tracking-wide hover:bg-studio-accent hover:text-white transition-all duration-300 transform hover:-translate-y-0.5">
                Join Us
             </button>
          </div>
